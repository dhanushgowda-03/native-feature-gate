...
class FlagAdminController {
    ...
    ResponseEntity<FlagResponse> create(@Valid @RequestBody CreateFlagRequest request) {
        ...
        FeatureFlag flag = new FeatureFlag(
            flagKey: request.flagKey, name: request.name,
            description: request.description, enabled: request.enabled,
            environment: properties.environment
        )
        request.rules?.each { ruleReq ->
            flag.rules.add(new FlagRule(flag: flag, strategy: ruleReq.strategy, parameters: ruleReq.parameters))
        }
        ...
        log.info("Flag created: {} in {}", saved.flagKey, properties.environment)
        return ResponseEntity.status(HttpStatus.CREATED).body(FlagResponse.from(saved))
    }

    @GetMapping
    ResponseEntity<List<FlagResponse>> list() {
        return ResponseEntity.ok(flagRepository.findAllByEnvironmentWithRules(properties.environment).collect { FlagResponse.from(it) })
    }
    ...
    ResponseEntity<List<FlagAuditLog>> auditHistory(@PathVariable String key,
        ...
        return ResponseEntity.ok(auditLogRepository.findByFlagKeyAndEnvironmentOrderByChangedAtDesc(
    ...
    private void audit(String flagKey, String changeType, String oldValue, String newValue) {
        try {
            auditLogRepository.save(new FlagAuditLog(
                flagKey: flagKey, environment: properties.environment,
                changeType: changeType, oldValue: oldValue, newValue: newValue
            ))
        } catch (Exception e) {
            log.warn("Audit log write failed for flag '{}': {}", flagKey, e.getMessage())
        }
    }
}
